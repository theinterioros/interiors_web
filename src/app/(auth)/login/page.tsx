      ...
      <main className="page flex-1 flex flex-col justify-center py-8">
        <div className="page-inner">
          <div className="mx-auto max-w-5xl">
            <div className={`grid gap-12 lg:grid-cols-2 ${isDesigner ? "lg:flex-row-reverse" : ""}`}>
              ...
              >
                <div
                  className={`mb-6 flex h-14 w-14 items-center justify-center rounded-2xl ${
                    isAdmin ? "bg-[var(--brand-light)] text-[var(--brand)]" : isDesigner ? "bg-teal-100 text-teal-700" : "bg-[var(--brand-light)] text-[var(--brand)]"
                  }`}
                >
                  {isAdmin ? <ShieldCheck className="h-7 w-7" /> : isDesigner ? <Building2 className="h-7 w-7" /> : <Home className="h-7 w-7" />}
                </div>
                <p className={`eyebrow mb-2 ${isDesigner ? "text-teal-600" : ""}`}>{isAdmin ? "Admin" : isDesigner ? "Designer" : "Homeowner"}</p>
                ...
                <p className="text-[var(--text-muted)] mb-6">
                  {isAdmin
                    ? "Access the admin dashboard to manage users, designers, and payments."
                    : isDesigner
                ...
                <ul className="space-y-3 text-sm text-[var(--text-muted)]">
                  {isAdmin ? (
                    <>
                      <li className="flex items-center gap-2">User & designer management</li>
                      <li className="flex items-center gap-2">Payment controls</li>
                      <li className="flex items-center gap-2">Settings & pricing</li>
                    </>
                  ) : isDesigner ? (
                    <>
                      <li className="flex items-center gap-2"><Building2 className="h-4 w-4 text-teal-600" /> Designer dashboard</li>
              ...
              >
                <div className="flex items-center gap-2 mb-4">
                  {isDesigner ? <Building2 className="h-4 w-4 text-teal-600" /> : <User className="h-4 w-4 text-[var(--brand)]" />}
                  <p className={isDesigner ? "eyebrow text-teal-600" : "eyebrow"}>{isAdmin ? "Admin sign in" : isDesigner ? "Designer sign in" : "Customer sign in"}</p>
                </div>
                ...
                <div>
                  <AuthLoginForm otpEnabled={settings.otpEnabled} isDesigner={isDesigner} />
                </div>
                <p className="mt-4 text-sm text-[var(--text-muted)]">
                  <Link
                    href={role ? `/forgot-password?role=${role}` : "/forgot-password"}
                    className={isDesigner ? "font-medium text-teal-600 hover:text-teal-700 hover:underline" : "font-medium text-[var(--brand)] hover:underline"}
                  >
                    Forgot password?
                  </Link>
                </p>
                <p className="mt-2 text-sm text-[var(--text-muted)]">
                  ...
                  <Link
                    ...
                    className={isDesigner ? "font-semibold text-teal-600 hover:text-teal-700 hover:underline" : "font-semibold text-[var(--brand)] hover:underline"}
                  >
                    Create an account
                  </Link>
                </p>
