...
function matchSearch(
  row: { title: string; customer_name: string | null; customer_email: string; designer_name: string | null; designer_email: string | null },
  q: string
) {
    ...
    <div className="space-y-8">
      <header>
        ...
        <p className="text-sm text-[var(--text-muted)]">
          All projects on the platform. Filter by status or search by title, customer, or designer.
        </p>
      </header>
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between mb-4">
        ...
        <div className="w-full sm:w-auto order-1 sm:order-2">
          <TableFilterBar
            value={q}
            placeholder="Search by title, customer, designer…"
      ...
      {filtered.length === 0 ? (
        <div className="rounded-lg border border-[var(--border)] bg-white p-6 sm:p-8 text-center text-[var(--text-muted)]">
          {q
            ? "No projects match your search. Try a different term or clear the filter."
            : filterStatus
