...
type Props = {
  ...
  showSource?: boolean;
  /** Controls how much detail to render */
  variant?: "full" | "rangeOnly" | "breakdownOnly";
};

export default function EstimatorResultSummary({
  result,
  showSource = false,
  variant = "full",
}: Props) {
  const isRangeOnly = variant === "rangeOnly";
  const isBreakdownOnly = variant === "breakdownOnly";
  ...
  return (
    <div className="space-y-4">
      ...
      {isRangeOnly ? (
        showSource ? <p className="text-xs uppercase text-[var(--text-muted)]">Source: {result.source}</p> : null
      ) : isBreakdownOnly ? (
        ...
        <>
          ...
          <div className="flex flex-wrap gap-2 text-sm text-[var(--text-muted)]">
            <span>
              Flat size:{" "}
              <strong className="text-[var(--foreground)]">{result.flatSizeSqFt.toLocaleString()} sq ft</strong>
            </span>
