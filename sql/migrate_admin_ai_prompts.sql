...
alter table admin_settings
  add column if not exists estimator_llm_provider text not null default 'OPENAI';

alter table admin_settings
  add column if not exists estimator_llm_model text;

alter table admin_settings
  add column if not exists visualization_llm_provider text not null default 'OPENAI';

alter table admin_settings
  add column if not exists visualization_llm_model text;

alter table admin_settings
  add column if not exists visualization_image_model text;
